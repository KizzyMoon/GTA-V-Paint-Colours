...
els.colourGrid.addEventListener("click",e=>{ const fav=e.target.closest("[data-favourite]");if(fav){toggleFavourite(Number(fav.dataset.favourite));return;} const copy=e.target.closest("[data-copy-code]");if(copy){const c=catalogue.find(x=>x.id===Number(copy.dataset.copyCode));copyText(`SetVehicleCustomPrimaryColour(vehicle, ${c.r}, ${c.g}, ${c.b})`,"FiveM code");return;} const details=e.target.closest("[data-details]");if(details)openDialog(Number(details.dataset.details)); });
...
document.querySelector("[data-open-picker]").addEventListener("click",()=>document.querySelector('[data-view="picker"]').click());
buildManufacturers();
render();
